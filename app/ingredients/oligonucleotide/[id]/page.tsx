...
export default function OligonucleotideDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const resolvedParams = use(params);
  const slug = resolvedParams.id;
  const oligoData = getIngredientBySlug("oligonucleotide", slug);

  if (!oligoData) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold">Oligonucleotide not found</h1>
          <Link href="/ingredients/oligonucleotide" className="text-primary hover:underline mt-4 inline-block">Back to Oligonucleotides</Link>
        </div>
      </div>
    );
  }
  
  const data = oligoData as { name: string; type: string; sequence: string; tm: string; length: string };
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="border-b border-border bg-background px-6 py-4">
        <div className="flex items-center gap-2 text-sm font-medium">
          ...
          <Link href="/ingredients/oligonucleotide" className="text-muted-foreground hover:text-foreground">Oligonucleotide</Link>
          <span className="text-muted-foreground">/</span>
              ...
              <div className="flex-1 overflow-clip px-px">
                <div className="flex items-center justify-between">
                  <div className="flex flex-col gap-1 w-[150px]">
                    ...
                    <div className="flex items-center py-2"><p className="text-base text-muted-foreground">Sequence</p></div>
                    <div className="flex items-center py-2"><p className="text-base text-muted-foreground">Tm</p></div>
                    <div className="flex items-center py-2"><p className="text-base text-muted-foreground">Length</p></div>
                  </div>
                  <div className="flex-1 flex flex-col gap-1">
                    ...
                    <div className="flex items-center gap-2 min-h-10 p-2 min-w-[85px]"><span className="text-xs font-mono break-all">{data.sequence}</span></div>
                    <div className="flex items-center gap-2 h-10 p-2 min-w-[85px]"><span className="text-sm">{data.tm}</span></div>
                    <div className="flex items-center gap-2 h-10 p-2 min-w-[85px]"><span className="text-sm">{data.length}</span></div>
                  </div>
                </div>
              </div>
            </div>
