...
import { oligonucleotides, nameToSlug } from "@/lib/ingredient-data";
...
export default function OligonucleotidePage() {
  const router = useRouter();
  const [isBulkImportOpen, setIsBulkImportOpen] = useState(false);

  const handleBulkImport = (data: BulkImportRow[]) => {
    console.log("Imported oligonucleotide data:", data);
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="border-b border-border bg-background px-6 py-4">
        <div className="flex items-center gap-2 text-sm font-medium">
          ...
          <span className="text-foreground">Oligonucleotide</span>
        </div>
      </div>
      ...
      <main className="p-8">
        ...
        <div className="border rounded-lg">
          <Table>
            ...
            <TableBody>
              {oligonucleotides.map((oligo, idx) => (
                <TableRow 
                  key={idx}
                  onClick={() => router.push(`/ingredients/oligonucleotide/${nameToSlug(oligo.name)}`)}
                  className="cursor-pointer"
                >
                  <TableCell className="font-medium">
                    {oligo.name}
                  </TableCell>
                  <TableCell>{oligo.type}</TableCell>
                  <TableCell className="font-mono text-xs">{oligo.sequence}</TableCell>
                  <TableCell>{oligo.lastEdited}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
