...
export default function PreservativeDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const resolvedParams = use(params);
  const slug = resolvedParams.id;
  const preservativeData = getIngredientBySlug("preservatives", slug);

  if (!preservativeData) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold">Preservative not found</h1>
          <Link href="/ingredients/preservatives" className="text-primary hover:underline mt-4 inline-block">Back to Preservatives</Link>
        </div>
      </div>
    );
  }
  
  const data = preservativeData as { name: string; concentration: string; purpose: string; storage: string };
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="border-b border-border bg-background px-6 py-4">
        <div className="flex items-center gap-2 text-sm font-medium">
          ...
          <Link href="/ingredients/preservatives" className="text-muted-foreground hover:text-foreground">Preservatives</Link>
          <span className="text-muted-foreground">/</span>
              ...
              <div className="flex-1 overflow-clip px-px">
                <div className="flex items-center justify-between">
                  <div className="flex flex-col gap-1 w-[150px]">
                    ...
                    <div className="flex items-center py-2"><p className="text-base text-muted-foreground">Starting Concentration</p></div>
                    <div className="flex items-center py-2"><p className="text-base text-muted-foreground">Purpose</p></div>
                    <div className="flex items-center py-2"><p className="text-base text-muted-foreground">Storage</p></div>
                  </div>
                  <div className="flex-1 flex flex-col gap-1">
                    ...
                    <div className="flex items-center gap-2 h-10 p-2 min-w-[85px]"><span className="text-sm">{data.concentration}</span></div>
                    <div className="flex items-center gap-2 h-10 p-2 min-w-[85px]"><span className="text-sm">{data.purpose}</span></div>
                    <div className="flex items-center gap-2 h-10 p-2 min-w-[85px]"><span className="text-sm">{data.storage}</span></div>
                  </div>
                </div>
              </div>
            </div>
