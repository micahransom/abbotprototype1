...
import { preservatives, nameToSlug } from "@/lib/ingredient-data";
...
export default function PreservativesPage() {
  ...
  const handleBulkImport = (data: BulkImportRow[]) => {
    console.log("Imported preservatives data:", data);
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="border-b border-border bg-background px-6 py-4">
        <div className="flex items-center gap-2 text-sm font-medium">
          ...
          <span className="text-foreground">Preservatives</span>
        </div>
      </div>
      ...
      <main className="p-8">
        ...
        <div className="border rounded-lg">
          <Table>
            ...
            <TableBody>
              {preservatives.map((preservative, idx) => (
                <TableRow key={idx}>
                  <TableCell className="font-medium">
                    <Link href={`/ingredients/preservatives/${nameToSlug(preservative.name)}`} className="hover:underline">{preservative.name}</Link>
                  </TableCell>
                  <TableCell>{preservative.concentration}</TableCell>
                  <TableCell>{preservative.lastEdited}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
