...
import { protocols, nameToSlug } from "@/lib/ingredient-data";
...
export default function ProtocolPage() {
  ...
  const handleBulkImport = (data: BulkImportRow[]) => {
    console.log("Imported protocol data:", data);
              ...
              {protocols.map((protocol, idx) => (
                <TableRow key={idx}>
                  <TableCell className="font-medium">
                    <Link href={`/ingredients/protocol/${nameToSlug(protocol.name)}`} className="hover:underline">{protocol.name}</Link>
                  </TableCell>
                  <TableCell>{protocol.type}</TableCell>
                  <TableCell>{protocol.lastEdited}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
