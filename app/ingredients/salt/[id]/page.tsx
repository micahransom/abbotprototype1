...
export default function SaltDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const resolvedParams = use(params);
  const slug = resolvedParams.id;
  const saltData = getIngredientBySlug("salt", slug);

  if (!saltData) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold">Salt not found</h1>
          <Link href="/ingredients/salt" className="text-primary hover:underline mt-4 inline-block">Back to Salts</Link>
        </div>
      </div>
    );
  }
  
  const data = saltData as { name: string; concentration: string; formula: string; mw: string };
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="border-b border-border bg-background px-6 py-4">
        <div className="flex items-center gap-2 text-sm font-medium">
          ...
          <Link href="/ingredients/salt" className="text-muted-foreground hover:text-foreground">Salt</Link>
          <span className="text-muted-foreground">/</span>
              ...
              <div className="flex-1 overflow-clip px-px">
                <div className="flex items-center justify-between">
                  <div className="flex flex-col gap-1 w-[150px]">
                    ...
                    <div className="flex items-center py-2"><p className="text-base text-muted-foreground">Formula</p></div>
                    <div className="flex items-center py-2"><p className="text-base text-muted-foreground">Molecular Weight</p></div>
                  </div>
                  <div className="flex-1 flex flex-col gap-1">
                    ...
                    <div className="flex items-center gap-2 h-10 p-2 min-w-[85px]"><span className="text-sm">{data.formula}</span></div>
                    <div className="flex items-center gap-2 h-10 p-2 min-w-[85px]"><span className="text-sm">{data.mw}</span></div>
                  </div>
                </div>
              </div>
            </div>
