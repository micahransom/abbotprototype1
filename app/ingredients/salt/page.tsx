...
import { salts, nameToSlug } from "@/lib/ingredient-data";
...
export default function SaltPage() {
  ...
  const handleBulkImport = (data: BulkImportRow[]) => {
    console.log("Imported salt data:", data);
              ...
              {salts.map((salt, idx) => (
                <TableRow key={idx}>
                  <TableCell className="font-medium">
                    <Link href={`/ingredients/salt/${nameToSlug(salt.name)}`} className="hover:underline">{salt.name}</Link>
                  </TableCell>
                  <TableCell>{salt.concentration}</TableCell>
                  <TableCell>{salt.lastEdited}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
