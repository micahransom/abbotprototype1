...
export default function StabilizerDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const resolvedParams = use(params);
  const slug = resolvedParams.id;
  const stabilizerData = getIngredientBySlug("stabilizers", slug);

  if (!stabilizerData) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold">Stabilizer not found</h1>
          <Link href="/ingredients/stabilizers" className="text-primary hover:underline mt-4 inline-block">Back to Stabilizers</Link>
        </div>
      </div>
    );
  }
  
  const data = stabilizerData as { name: string; concentration: string; purpose: string; storage: string };
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="border-b border-border bg-background px-6 py-4">
        <div className="flex items-center gap-2 text-sm font-medium">
          ...
          <Link href="/ingredients/stabilizers" className="text-muted-foreground hover:text-foreground">Stabilizers</Link>
          <span className="text-muted-foreground">/</span>
