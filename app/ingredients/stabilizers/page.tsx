...
import { stabilizers, nameToSlug } from "@/lib/ingredient-data";
...
export default function StabilizersPage() {
  ...
  const handleBulkImport = (data: BulkImportRow[]) => {
    console.log("Imported stabilizers data:", data);
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="border-b border-border bg-background px-6 py-4">
        <div className="flex items-center gap-2 text-sm font-medium">
          ...
          <span className="text-foreground">Stabilizers</span>
        </div>
      </div>
      ...
      <main className="p-8">
        ...
        <div className="border rounded-lg">
          <Table>
            ...
            <TableBody>
              {stabilizers.map((stabilizer, idx) => (
                <TableRow key={idx}>
                  <TableCell className="font-medium">
                    <Link href={`/ingredients/stabilizers/${nameToSlug(stabilizer.name)}`} className="hover:underline">{stabilizer.name}</Link>
                  </TableCell>
                  <TableCell>{stabilizer.concentration}</TableCell>
                  <TableCell>{stabilizer.lastEdited}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
