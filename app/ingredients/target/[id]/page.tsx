...
export default function TargetDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const targetData = getIngredientBySlug("target", slug);
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/ingredients/target" className="text-primary hover:underline mt-4 inline-block">Back to Targets</Link>
        </div>
      </div>
    );
  }
  
  const data = targetData as { name: string; type: string; gene: string; organism: string };
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="border-b border-border bg-background px-6 py-4">
        <div className="flex items-center gap-2 text-sm font-medium">
          ...
          <Link href="/ingredients/target" className="text-muted-foreground hover:text-foreground">Target</Link>
          <span className="text-muted-foreground">/</span>
              ...
              <div className="flex-1 overflow-clip px-px">
                <div className="flex items-center justify-between">
                  <div className="flex flex-col gap-1 w-[150px]">
                    ...
                    <div className="flex items-center py-2"><p className="text-base text-muted-foreground">Gene Target</p></div>
                    <div className="flex items-center py-2"><p className="text-base text-muted-foreground">Organism</p></div>
                  </div>
                  <div className="flex-1 flex flex-col gap-1">
                    ...
                    <div className="flex items-center gap-2 h-10 p-2 min-w-[85px]"><span className="text-sm">{data.gene}</span></div>
                    <div className="flex items-center gap-2 h-10 p-2 min-w-[85px]"><span className="text-sm">{data.organism}</span></div>
                  </div>
                </div>
              </div>
            </div>
