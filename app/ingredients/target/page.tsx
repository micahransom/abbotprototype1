...
import { targets, nameToSlug } from "@/lib/ingredient-data";
...
export default function TargetPage() {
  ...
  const handleBulkImport = (data: BulkImportRow[]) => {
    console.log("Imported target data:", data);
              ...
              {targets.map((target, idx) => (
                <TableRow key={idx}>
                  <TableCell className="font-medium">
                    <Link href={`/ingredients/target/${nameToSlug(target.name)}`} className="hover:underline">{target.name}</Link>
                  </TableCell>
                  <TableCell>{target.type}</TableCell>
                  <TableCell>{target.lastEdited}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
