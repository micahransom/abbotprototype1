...
export default function ThermocyclerDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const resolvedParams = use(params);
  const slug = resolvedParams.id;
  const thermocyclerData = getIngredientBySlug("thermocycler", slug);

  if (!thermocyclerData) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold">Thermocycler not found</h1>
          <Link href="/ingredients/thermocycler" className="text-primary hover:underline mt-4 inline-block">Back to Thermocyclers</Link>
        </div>
      </div>
    );
  }
  
  const data = thermocyclerData as { name: string; serial: string; location: string; status: string };
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="border-b border-border bg-background px-6 py-4">
        <div className="flex items-center gap-2 text-sm font-medium">
          ...
          <Link href="/ingredients/thermocycler" className="text-muted-foreground hover:text-foreground">Thermocycler</Link>
          <span className="text-muted-foreground">/</span>
          <span className="text-foreground">{data.name} ({data.serial})</span>
        </div>
      </div>
              ...
              <div className="flex-1 overflow-clip px-px">
                <div className="flex items-center justify-between">
                  <div className="flex flex-col gap-1 w-[150px]">
                    ...
                    <div className="flex items-center py-2"><p className="text-base text-muted-foreground">Serial Number</p></div>
                    <div className="flex items-center py-2"><p className="text-base text-muted-foreground">Location</p></div>
                    <div className="flex items-center py-2"><p className="text-base text-muted-foreground">Status</p></div>
                  </div>
                  <div className="flex-1 flex flex-col gap-1">
                    ...
                    <div className="flex items-center gap-2 h-10 p-2 min-w-[85px]"><span className="text-sm">{data.serial}</span></div>
                    <div className="flex items-center gap-2 h-10 p-2 min-w-[85px]"><span className="text-sm">{data.location}</span></div>
                    <div className="flex items-center gap-2 h-10 p-2 min-w-[85px]"><span className="text-sm">{data.status}</span></div>
                  </div>
                </div>
              </div>
            </div>
