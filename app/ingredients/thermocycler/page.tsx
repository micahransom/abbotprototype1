...
import { thermocyclers, nameToSlug } from "@/lib/ingredient-data";
...
export default function ThermocyclerPage() {
  const [isBulkImportOpen, setIsBulkImportOpen] = useState(false);

  const handleBulkImport = (data: BulkImportRow[]) => {
    console.log("Imported thermocycler data:", data);
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="border-b border-border bg-background px-6 py-4">
        <div className="flex items-center gap-2 text-sm font-medium">
          ...
          <span className="text-foreground">Thermocycler</span>
        </div>
      </div>
      ...
      <main className="p-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={() => setIsBulkImportOpen(true)}>Bulk Import</Button>
            <Tooltip content="Not Yet Built" side="bottom">
              <Button className="bg-primary text-primary-foreground hover:bg-primary/90 font-semibold" size="sm">Add Ingredient</Button>
            </Tooltip>
          </div>
        </div>
        ...
        <div className="border rounded-lg">
          <Table>
            ...
            <TableBody>
              {thermocyclers.map((thermocycler, idx) => (
                <TableRow key={idx}>
                  <TableCell className="font-medium">
                    <Link href={`/ingredients/thermocycler/${nameToSlug(thermocycler.name)}-${thermocycler.serial}`} className="hover:underline">{thermocycler.name}</Link>
                  </TableCell>
                  <TableCell>{thermocycler.serial}</TableCell>
                  <TableCell>{thermocycler.lastEdited}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
