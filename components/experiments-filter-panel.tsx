...
interface FilterPanelProps {
  ...
  availableExperimentTypes: Array<{ name: string; count: number }>;
  availableIngredients: Array<{ name: string; count: number }>;
}

export function ExperimentsFilterPanel({
  isOpen,
  onClose,
  filters,
  onFiltersChange,
  ...
  const toggleExperimentType = (type: string) => {
    const newTypes = filters.experimentTypes.includes(type)
      ? filters.experimentTypes.filter((t) => t !== type)
      : [...filters.experimentTypes, type];
    onFiltersChange({ ...filters, experimentTypes: newTypes });
  };
      ...
      <div className="relative w-full max-w-[500px] h-full bg-background shadow-[0px_20px_25px_-5px_rgba(2,6,23,0.1),0px_8px_10px_-6px_rgba(2,6,23,0.1)] rounded-l-2xl overflow-hidden flex flex-col">
        ...
        <div className="flex-none px-6 pt-6 pb-4 border-b border-border">
          ...
          <div className="flex flex-wrap gap-2 items-center min-h-[20px]">
            {appliedFiltersCount === 0 ? (
              ...
              <>
                {filters.experimentTypes.map((type) => (
                  <Badge
                    key={type}
                    variant="secondary"
                    className="cursor-pointer hover:bg-secondary/80"
                    onClick={() => toggleExperimentType(type)}
                  >
                    {type}
        ...
        <div className="flex-1 overflow-y-auto">
          <Accordion type="multiple" defaultValue={["target", "lastEdited"]}>
            {/* Experiment Type */}
            <AccordionItem value="experimentType">
              <AccordionTrigger>Experiment Type</AccordionTrigger>
              <AccordionContent>
                <div className="space-y-3">
                  {availableExperimentTypes.map((type) => (
                    <div key={type.name} className="flex items-center gap-2">
                      <Checkbox
                        id={`type-${type.name}`}
                        checked={filters.experimentTypes.includes(type.name)}
                        onCheckedChange={() => toggleExperimentType(type.name)}
                      />
                      <label
