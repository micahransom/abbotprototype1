// Centralized ingredient database for the prototype
...
export interface Buffer extends BaseIngredient {
  concentration: string;
  ph: string;
  storage: string;
}

export interface Enzyme extends BaseIngredient {
  activity: string;
  storage: string;
  supplier: string;
}

export interface Oligonucleotide extends BaseIngredient {
  type: string;
  sequence: string;
  tm: string;
  length: string;
...
export interface Preservative extends BaseIngredient {
  concentration: string;
  purpose: string;
  storage: string;
}

export interface PrimerProbePair extends BaseIngredient {
  primer1: string;
  primer2: string;
  probe: string;
}

export interface Protocol extends BaseIngredient {
  type: string;
  duration: string;
  temperature: string;
}

export interface Salt extends BaseIngredient {
  concentration: string;
  formula: string;
  mw: string;
}
...
export interface Target extends BaseIngredient {
  type: string;
  gene: string;
  organism: string;
}

export interface Thermocycler extends BaseIngredient {
  serial: string;
  location: string;
  status: string;
}
...
/**
 * Get ingredient by category and ID
 */
export function getIngredientById(category: IngredientCategory, id: string) {
  const ingredients = getIngredientsByCategory(category);
  return ingredients.find((item) => item.id === id);
}

/**
 * Get ingredient by category and name slug
 */
export function getIngredientBySlug(category: IngredientCategory, slug: string) {
  const ingredients = getIngredientsByCategory(category);
  ...
  return ingredients.find((item) => nameToSlug(item.name) === slug);
}

/**
 * Get ingredient names for dropdown options
 */
export function getIngredientNames(category: IngredientCategory): string[] {
  const ingredients = getIngredientsByCategory(category);
  return ingredients.map((item) => item.name);
}

/**
 * Get ingredient options for dropdown (id + name pairs)
 */
export function getIngredientOptions(category: IngredientCategory): Array<{ id: string; name: string }> {
  const ingredients = getIngredientsByCategory(category);
  return ingredients.map((item) => ({ id: item.id, name: item.name }));
}

/**
 * Search ingredients by name across category
 */
export function searchIngredients(category: IngredientCategory, searchTerm: string) {
  const ingredients = getIngredientsByCategory(category);
